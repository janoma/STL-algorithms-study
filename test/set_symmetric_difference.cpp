...
#define BOOST_TEST_MAIN set_symmetric_difference tests
...
BOOST_AUTO_TEST_SUITE( set_symmetric_difference_test_suite )
...
BOOST_AUTO_TEST_CASE( empty_primary_input_default_compare )
{
    ...
    std::set_symmetric_difference(std::begin(primary), std::end(primary), std::begin(secondary), std::end(secondary), std::back_inserter(output));
...
BOOST_AUTO_TEST_CASE( empty_primary_input_custom_compare )
{
    ...
    std::set_symmetric_difference(std::begin(primary), std::end(primary), std::begin(secondary), std::end(secondary), std::back_inserter(output), std::greater<int>());
...
BOOST_AUTO_TEST_CASE( empty_primary_input_conversion_compare )
{
    ...
    std::set_symmetric_difference(std::begin(primary), std::end(primary), std::begin(secondary), std::end(secondary), std::back_inserter(output), std::less<double>());
...
BOOST_AUTO_TEST_CASE( same_type_inputs_custom_compare )
{
    ...
    std::set_symmetric_difference(std::begin(primary), std::end(primary), std::begin(secondary), std::end(secondary), std::back_inserter(output), comp);
...
BOOST_AUTO_TEST_CASE( different_type_inputs_default_compare )
{
    ...
    const std::vector<int> expected{1, 1, 2, 3, 4, 5, 6};
    BOOST_CHECK_EQUAL_COLLECTIONS(std::begin(output), std::end(output), std::begin(expected), std::end(expected));
}
...
BOOST_AUTO_TEST_CASE( different_type_inputs_custom_compare )
{
    ...
    const std::vector<int> expected{2, 5, 6};
    BOOST_CHECK_EQUAL_COLLECTIONS(std::begin(output), std::end(output), std::begin(expected), std::end(expected));
}
...
BOOST_AUTO_TEST_SUITE_END( /* set_symmetric_difference_test_suite */ )
